...
static auto const &get_keyword_list() {
    static std::unordered_map<std::string_view, keyword_data> keyword_list = {
        ...
        {"if", {add_token, token_type::keyword, "if"}},
        {"while", {add_token, token_type::keyword, "while"}},
        {"else", {add_token, token_type::keyword, "else"}},
        {"class", {add_token, token_type::keyword, "class"}},
        {"for", {add_token, token_type::keyword, "for"}},
...
std::vector<token> Interpret(std::string_view input_arg) {
    ...
    for (; idx < input.size(); idx++) {
        ...
        case '\t':
            break;
        case '\n':
            newlines.push_back(token_list.size());
            break;
        case '.':
        ...
        case '<':
            if (input[idx + 1] == '=') {
                add_token(token_type::is_less_than_or_equal);
                idx++;
                break;
            }
            add_token(token_type::is_less_than);
            break;
        case '>':
            if (input[idx + 1] == '=') {
                add_token(token_type::is_greater_or_equal);
                idx++;
                break;
            }
            add_token(token_type::is_greater_than);
            break;
        case '*':
        ...
        case '!':
            if (input[idx + 1] == '=') {
                ...
                report_error("! operator not imlemented");
                goto stop;
            }
        ...
        case ';':
            ...
            break;
        case '{':
            add_token(token_type::open_curly);
            break;
        case '}':
            add_token(token_type::closing_curly);
            break;
        case '\0':
